...
export default function ChatBox() {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      if (data.length === 0) {
        ...
        if (page === 1) {
          setUsers(data.filter(u => u.userRole === "ROLE_TECHNICIAN"));
        } else {
          setUsers(prev => [...prev, ...data.filter(u => u.userRole === "ROLE_TECHNICIAN")]);
        }
      }
    } catch (err) {
  ...
  return (
    <div style={{ display: "flex", height: "40vh", marginTop: "50px", marginBottom:"100px", border: "1px solid #ccc", borderRadius: "10px" }}>
      <Sidebar
        users={users}
