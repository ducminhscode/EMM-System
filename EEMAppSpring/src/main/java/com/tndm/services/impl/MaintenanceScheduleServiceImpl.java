        ...
        for (MaintenanceSchedule maintenance : listMaintenances) {
            ...
            for (MaintenanceAssignment mainAssign : listAssigns) {
                if (mainAssign.getIsNotify().equals(Boolean.FALSE)) {
                    mailSer.sendMail(mainAssign.getTechnicianId().getUser().getEmail(), maintenance.getMaintenanceStatus().equals("Chưa bảo trì") ? "Tới hạn bảo trì thiết bị" : "Quá hạn bảo trì thiết bị", "Thiết bị: " + maintenance.getDeviceId().getName() + "\n" + "Cơ sở: " + maintenance.getDeviceId().getFacilityId().getName());
